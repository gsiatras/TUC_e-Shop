...
const allowedOrigins = ['http://localhost:3001', 'http://172.17.0.1:3001', 'http:34.118.15.144:3001'];
...
app.post('/orders', async (req, res) => {
    ...
    const session = await stripe.checkout.sessions.create({
        ...
        success_url: 'http://34.118.15.144:3001/customer/cart?success=1',
        cancel_url: 'http://34.118.15.144:3001/customer/cart?canceled=1',
        metadata: {orderIds: orderIds.join(','),},
    })

    res.setHeader('Access-Control-Allow-Origin', 'http://34.118.15.144:3001');
    res.json({
        url:session.url,
    })
});
